...
fn find_json_for_eml(eml_path: &Path) -> Option<PathBuf> {
    // two common patterns: "name.eml.json" and "name.json"
    let alt1 = eml_path.with_extension("json"); // e.g., name.json
    if alt1.exists() {
        return Some(alt1);
    }
    if let Some(parent) = eml_path.parent() {
        if let Some(fname) = eml_path.file_name().and_then(|s| s.to_str()) {
            let alt2 = parent.join(format!("{}.json", fname)); // e.g., name.eml.json
...
fn list_queue_entries(maildrop_dir: &Path) -> Result<Vec<QueueEntry>> {
    ...
    for e in read_dir.filter_map(|r| r.ok()) {
        ...
        if p.extension().and_then(|s| s.to_str()).map(|s| s == "eml").unwrap_or(false) {
            let filename = p.file_name().and_then(|s| s.to_str()).unwrap_or_default().to_string();
            let json_path = find_json_for_eml(&p);
            let control = if let Some(ref jp) = json_path {
                match fs::read_to_string(jp) {
                    Ok(s) => serde_json::from_str::<QueueControl>(&s).unwrap_or_else(|_| QueueControl::default_with_timestamp(0)),
                    Err(_) => QueueControl::default_with_timestamp(0),
                }
            } else { QueueControl::default_with_timestamp(0) };
            ...
            let domain = envelope_to.as_ref().and_then(|r| r.rfind('@').and_then(|i| Some(r[i+1..].to_lowercase())));

            out.push(QueueEntry { filename, eml_path: p, json_path, control, created_at, envelope_to, domain });
        }
    }
    Ok(out)
}
...
fn count_inflight_by_domain(maildrop_dir: &Path) -> Result<HashMap<String, usize>> {
    ...
    let rd = match fs::read_dir(&inflight_dir) { Ok(r) => r, Err(_) => return Ok(counts) };
...
pub fn claim_one_with_limit(maildrop_dir: &Path, per_destination_limit: usize) -> Result<Option<(PathBuf, PathBuf)>> {
    ...
    entries.retain(|e| if let Some(nt) = e.control.next_try { nt <= now } else { true });

    // sort by priority desc, created_at asc
    entries.sort_by(|a, b| b.control.priority.cmp(&a.control.priority).then_with(|| a.created_at.cmp(&b.created_at)));
    ...
    for ent in entries {
        ...
        if cur >= per_destination_limit { continue; }
        ...
        let inflight_json_dst = if let Some(json_src) = &json_src_opt {
            ...
            let dst = inflight_dir.join(format!("{}.json", eml_dst.file_name().and_then(|s| s.to_str()).unwrap_or("msg")));
...
pub fn collect_metrics(maildrop_dir: &Path) -> Result<QueueMetrics> {
    let queue_dir = maildrop_dir.join("queue");
    let inflight_dir = maildrop_dir.join("inflight");
    let outbound_root = maildrop_dir.parent().unwrap_or_else(|| Path::new("./outbound")).to_path_buf();
    let sent_dir = outbound_root.join("sent");
    let failed_dir = outbound_root.join("failed");
    let dead_dir = outbound_root.join("dead");

    let queued = fs::read_dir(&queue_dir).map(|r| r.filter_map(|e| e.ok()).filter(|e| e.path().extension().and_then(|s| s.to_str())==Some("eml")).count()).unwrap_or(0);
    let inflight = fs::read_dir(&inflight_dir).map(|r| r.filter_map(|e| e.ok()).filter(|e| e.path().extension().and_then(|s| s.to_str())==Some("eml")).count()).unwrap_or(0);
    let sent = fs::read_dir(&sent_dir).map(|r| r.filter_map(|e| e.ok()).filter(|e| e.path().extension().and_then(|s| s.to_str())==Some("eml")).count()).unwrap_or(0);
    let failed = fs::read_dir(&failed_dir).map(|r| r.filter_map(|e| e.ok()).filter(|e| e.path().extension().and_then(|s| s.to_str())==Some("eml")).count()).unwrap_or(0);
    let dead = fs::read_dir(&dead_dir).map(|r| r.filter_map(|e| e.ok()).filter(|e| e.path().extension().and_then(|s| s.to_str())==Some("eml")).count()).unwrap_or(0);

    let per_dest = count_inflight_by_domain(maildrop_dir)?;

    Ok(QueueMetrics { queued, inflight, sent, failed, dead, per_destination_inflight: per_dest })
}
...
pub fn dead_letter_cleanup(maildrop_dir: &Path, older_than_secs: i64) -> Result<usize> {
    ...
    let rd = match fs::read_dir(&failed_dir) { Ok(r) => r, Err(_) => return Ok(moved) };
    let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs() as i64;
    for e in rd.filter_map(|r| r.ok()) {
        let p = e.path();
        if !p.is_file() { continue; }
        if p.extension().and_then(|s| s.to_str()) != Some("eml") { continue; }

        // find control json
        let json_path = find_json_for_eml(&p);
        let created_at = if let Some(ref jp) = json_path { 
            match fs::read_to_string(jp).ok().and_then(|s| serde_json::from_str::<QueueControl>(&s).ok()) {
                Some(c) if c.created_at != 0 => c.created_at,
                _ => 0,
            }
        } else { 0 };

        let age = if created_at != 0 { now - created_at } else {
            match fs::metadata(&p).and_then(|m| m.modified()) {
                Ok(t) => t.duration_since(UNIX_EPOCH).map(|d| d.as_secs() as i64).unwrap_or(0),
                Err(_) => 0,
            }
...
mod tests {
    ...
    fn test_dead_letter_cleanup_moves_old() -> Result<()> {
        ...
        ctl.created_at = (SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() as i64) - (60*60*24*40);
