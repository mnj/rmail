...
function App() {
  ...
  const selectedIds = checked.length ? checked : selected ? [selected.uid] : [];
  ...
  async function bulk(action: string) {
    ...
    await api(`/api/folders/${encodeURIComponent(folder)}/messages/bulk`, { method: 'POST', body: JSON.stringify({ action, uids: selectedIds }) });
    setSelected(null);
    await refresh();
  }
  ...
  return (
    <main className={`app mobile-${mobileView}`}>
      ...
      <section className="mailbox">
        ...
        <div className="toolbar"><strong>{title}</strong><button className="icon" onClick={() => bulk('archive')} title="Archive"><Archive size={18} /></button><button className="icon" onClick={() => bulk('delete')} title="Delete"><Trash2 size={18} /></button><button className="icon" onClick={() => bulk('mark_read')} title="Mark read"><MailOpen size={18} /></button><button className="icon" onClick={() => bulk('mark_unread')} title="Mark unread"><Mail size={18} /></button></div>
      ...
      <article className="reader">{selected ? <><button className="back mobile-only" onClick={() => setMobileView('list')}>Back</button><h2>{selected.subject || '(no subject)'}</h2><div className="meta">From {selected.from || '(unknown)'} to {selected.to || address}</div><pre>{selected.text_body}</pre></> : <div className="empty">Select a message</div>}</article>
    </main>
  );
}
